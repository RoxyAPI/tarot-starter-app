...
/**
 * The single Roxy SDK client for the whole app. `createRoxy` sets the base URL and injects the auth header on every request, so there is no base URL to configure and no generated schema to keep in sync: the SDK ships its own types. One key unlocks every domain in the catalog.
 *
